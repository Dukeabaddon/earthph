...
export default function EventModal({ event, isOpen, onClose, position }) {
  ...
  useEffect(() => {
    function handleOutsideClick(e) {
      if (!isOpen) return;
      const modal = modalRef.current;
      if (modal && !modal.contains(e.target)) onClose();
    }
  ...
  if (event.datetime) {
    try {
      ...
      if (!isNaN(testDate.getTime())) dateTime = formatDateTime(event.datetime);
    } catch (err) { console.error('[EventModal] Date formatting error:', err); }
  }
    ...
    <>
      ...
      <div ref={modalRef} className="fixed z-[1200] w-80 shadow-2xl rounded-lg overflow-hidden" style={{ top: `${adjustedPosition.top}px`, left: `${adjustedPosition.left}px` }} role="dialog" aria-modal="true" aria-labelledby="modal-title" onClick={(e) => e.stopPropagation()}>
        <div className="bg-black px-3 py-2 flex items-center justify-between">
          <h2 id="modal-title" className="text-white text-xs font-semibold uppercase">Earthquake Details</h2>
          <button onClick={onClose} className="text-gray-300 hover:text-white text-lg leading-none w-5 h-5 flex items-center justify-center" aria-label="Close">×</button>
        </div>
        <div className="bg-white">
          ...
          <div className="px-3 py-1 bg-gray-50 border-t border-gray-200"><p className="text-xs text-gray-600">Source: <span className="font-medium">PHIVOLCS</span></p></div>
        </div>
      </div>
    </>
  );
